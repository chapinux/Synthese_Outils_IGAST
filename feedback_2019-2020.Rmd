...
Dans la séance de synthèse,les 19 étudiants ont répondu à diverses questions ouvertes dont voici les réponses,illustrées sous la forme de nuages de mots quand c'est possible. 
...
Matlab etst utilisé en aprallèle d'Octave, sa version opensource très similaire, pour ceux qui n'ont pas de licence ou qui n'ont pas pu venir à l'ENSG.
...
## Interprétation
 ...
 Les étudiants distinguent le langage de programmation de l'IDE utilisé pour développer : R et RStudio, python et Spyder. Aucun n'a mentionné d'outil bureautique de "rendu" (traitement de texte, diaporama), à l'exception de LaTeX.
...
```{r cloud2,cache=TRUE,echo=FALSE,message=FALSE,warning=FALSE,fig.show="hold",out.width="50%"}
...
rawCrits <- "Facilité de diffusion,Frugalité,Robustesse,Qualité de la documentation,Qualité de la documentation,tutorial d'utilisation,Qualité de la documentation,Frugalité,Frugalité,Rapidité,Open source,Facilité d'utilisation,Facilité d'installation,Open source,Utilisation dans le milieu pro,quantité d'extensions,Qualité de la documentation,Intuitivité,Extensibilité,Open source,Qualité de l'interface,Open source,Stabilité,Rapidité,Répondre au besoin,Robustesse,Qualité de l'interface,Polyvalence,Facilité d'utilisation,Rapidité,Accessibilité,Rapidité,Simplicité,Intuitivité,Qualité de la documentation,Qualité de la documentation,Rapidité,Qualité de l'interface,Qualité de l'interface"
...
- _Pour ma part je n'ai pas de préférance chacune on leurs avantages_
...
La GUI remporte une large adhésion. Je crois que c'est parce qu'en dehors du trio "Qgis-postgis-python", les outils sont "découverts", c'est-à-dire abordés superficiellement et en peu de temps, or pour avoir un aperçu des fonctionnalités d'un outil, une GUI fait mieux l'affaire.

Les étudiant.e.s semblent friands d'interfaces ergonomiques et esthétiques, pour pouvoir cliquer un peu partout et voir ce que ça fait. 
...
Je demandais une selection non ordonnée parmi les outils à disposition.
...
Transformer ici signifie : changer de format, de rojection, ou de nature (par exemple vectoriser des rasters ou rasteriser des données vecteur)^.
...
pour le calcul mesure ,enrichissement : 
...
python
pgsql
Postgres,Qgis,R,Python
ArcGIS
Numpy
QGIS/Python/R/PostGresSQL
Python
r
python
Excel
...
Exporte et mise à disposition des données
...
Leaflet/OpenLayers
Alwaysdata
Alwaysdata pour héberger un site
Pyhton : with open('nom_fichier',"w") as f
WKT,Geojson
Webmapping
QgisToThreeJs
Geoserver
